...
decay_list = [0.999, 0.99, 0.95, 0.9]
...
for decay in decay_list:
    print(f"Running decay = {decay} ...")
    SR_log, S_std_log, Goal_std_log = run_simulation(decay)
...
plt.subplot(1, 3, 1)
for decay in decay_list:
    plt.plot(results[decay]["SR"], label=f"decay={decay}")
...
plt.subplot(1, 3, 2)
for decay in decay_list:
    plt.plot(results[decay]["S_std"], label=f"decay={decay}")
...
plt.subplot(1, 3, 3)
for decay in decay_list:
    plt.plot(results[decay]["Goal_std"], label=f"decay={decay}")
plt.axhline(y=0.001, linestyle='--', color='red', label='threshold')
plt.title("Goal Std (Will)")
plt.legend()

plt.tight_layout()
plt.savefig(f"{BASE_PATH}\\philia_v19_decay_scan.png", dpi=150)
plt.show()
